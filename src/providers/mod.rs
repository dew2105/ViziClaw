...
/// 3. Generic fallback variables (`VIZICLAW_API_KEY`, `API_KEY`)
///
...
fn resolve_api_key(name: &str, api_key: Option<&str>) -> Option<String> {
    ...
    for env_var in ["VIZICLAW_API_KEY", "API_KEY"] {
        if let Ok(value) = std::env::var(env_var) {
            let value = value.trim();
            if !value.is_empty() {
                return Some(value.to_string());
...
pub fn create_provider(name: &str, api_key: Option<&str>) -> anyhow::Result<Box<dyn Provider>> {
    ...
    match name {
        ...
        _ => anyhow::bail!(
            "Unknown provider: {name}. Check README for supported providers or run `viziclaw onboard --interactive` to reconfigure.\n\
