...
// in `~/.viziclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.viziclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(viziclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: viziclaw_dir.join(".secret_key"),
            enabled,
        }
    }
