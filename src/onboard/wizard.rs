...
const BANNER: &str = r"
    ...
    Total visibility. Total control. 100% Rust.
...
pub fn run_wizard() -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to ViziClaw — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 8, "Channels (How You Talk to ViziClaw)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("VIZICLAW_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to ViziClaw)");
...
/// Use `viziclaw onboard` or `viziclaw onboard --api-key sk-... --provider openrouter --memory sqlite`.
/// Use `viziclaw onboard --interactive` for the full wizard.
...
pub fn run_quick_setup(
    ...
    let workspace_dir = viziclaw_dir.join("workspace");
    let config_path = viziclaw_dir.join("config.toml");
    ...
    if api_key.is_none() {
        ...
        println!("    2. Or edit:           ~/.viziclaw/config.toml");
        println!("    3. Chat:              viziclaw agent -m \"Hello!\"");
        println!("    4. Gateway:           viziclaw gateway");
    } else {
        println!("    1. Chat:     viziclaw agent -m \"Hello!\"");
        println!("    2. Gateway:  viziclaw gateway");
        println!("    3. Status:   viziclaw status");
    }
    println!();
...
fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    let home = directories::UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    let default_dir = home.join(".viziclaw");
    ...
    let viziclaw_dir = if use_default {
        default_dir
    } else {
...
fn setup_provider() -> Result<(String, String, String)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("ViziClaw works with ANY API that speaks the OpenAI chat completions format.");
        ...
        || provider_name == "google-gemini"
    {
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("ViziClaw will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how ViziClaw connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("ViziClaw uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("ViziClaw can encrypt API keys stored in config.toml.");
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how ViziClaw stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to ViziClaw from anywhere.");
        ...
        match choice {
            0 => {
                ...
                println!(
                    ...
                    style("— talk to ViziClaw from Telegram").dim()
                );
            ...
            1 => {
                ...
                println!(
                    ...
                    style("— talk to ViziClaw from Discord").dim()
                );
            ...
            2 => {
                ...
                println!(
                    ...
                    style("— talk to ViziClaw from Slack").dim()
                );
            ...
            3 => {
                ...
                print_bullet("ViziClaw reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            5 => {
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("viziclaw-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via VIZICLAW_WHATSAPP_APP_SECRET env var
                    allowed_numbers,
                });
            }
            6 => {
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("ViziClaw is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if has_channels {
        ...
        println!("       {}", style("viziclaw channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("viziclaw agent -m \"Hello, ViziClaw!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("viziclaw agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("viziclaw status").yellow());
...
mod tests {
    ...
    fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = fs::read_to_string(tmp.path().join("IDENTITY.md")).unwrap();
        assert!(
            identity.contains("**Name:** ViziClaw"),
            "should default agent name to ViziClaw"
        );
    ...
    fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = fs::read_to_string(tmp.path().join("SOUL.md")).unwrap();
        assert!(soul.contains("ViziClaw-v2"));
    }
