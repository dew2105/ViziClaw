...
const OPEN_SKILLS_SYNC_MARKER: &str = ".viziclaw-open-skills-sync";
const OPEN_SKILLS_SYNC_INTERVAL_SECS: u64 = 60 * 60 * 24 * 7;

/// A skill is a user-defined or community-built capability.
/// Skills live in `~/.viziclaw/workspace/skills/<name>/SKILL.md`
...
fn open_skills_enabled() -> bool {
    if let Ok(raw) = std::env::var("VIZICLAW_OPEN_SKILLS_ENABLED") {
...
fn resolve_open_skills_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("VIZICLAW_OPEN_SKILLS_DIR") {
    ...
    if !readme.exists() {
        std::fs::write(
            ...
            "# ViziClaw Skills\n\n\
             ...
             ```bash\n\
             viziclaw skills install <github-url>\n\
             viziclaw skills list\n\
             ```\n",
        )?;
    }
    ...
    match command {
        crate::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                println!();
                println!("  Create one: mkdir -p ~/.viziclaw/workspace/skills/my-skill");
                println!("              echo '# My Skill' > ~/.viziclaw/workspace/skills/my-skill/SKILL.md");
                println!();
                println!("  Or install: viziclaw skills install <github-url>");
            } else {
        ...
        crate::SkillCommands::Install { source } => {
            ...
            if source.starts_with("https://") || source.starts_with("http://") {
                ...
                if output.status.success() {
                    ...
                    println!("  Restart `viziclaw channel start` to activate.");
                } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.viziclaw");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.viziclaw/skills"));
    }
