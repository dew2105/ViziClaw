...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/dew2105/ViziClaw/security/advisories/new)
...
ViziClaw implements defense-in-depth security:
...
ViziClaw Docker images follow CIS Docker Benchmark best practices:
...
docker build -t viziclaw .
docker inspect --format='{{.Config.User}}' viziclaw
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace viziclaw gateway
```
